<template>
  <div class="d-flex flex-column justify-content-start vh-100 w-100 bg-body-tertiary overflow-hidden">
    ...
    <!-- Main Body -->
    <div class="d-flex flex-column p-3 pt-0" style="height: fit-content !important">
    ...
    <div class="hstack w-100 h-100 px-3 py-2 gap-2 overflow-hidden">
      ...
      <div class="card w-100 h-100 shadow-sm">
        ...
        <div class="card-body overflow-auto list-group p-0 rounded-0">
          ...
          <button v-for="(book, index) in books" type="button" class="d-flex align-items-center list-group-item list-group-item-action" :key="index" @click="selectedBookId = book.id" data-bs-toggle="modal" data-bs-target="#viewItem">
        ...
        <div class="modal-body">
          <form class="d-flex flex-column text-start gap-3 h-100" @submit.prevent="fetchItems">
            <div class="d-flex flex-column">
              ...
              <select class="form-select" name="campus" id="campus" v-model="params.campus">
  ...
  <div class="modal fade" id="viewItem" tabindex="-1"><information-page :id="selectedBookId" @reserve="reserve()" /></div>
  <div class="modal fade" id="reserve" tabindex="-1" v-if="selectedBookId"><reservation-page :bookId="selectedBookId" :data="reservationDetails" /></div>
</template>
...
export default {
  ...
  methods: {
    ...
    async fetchItems() {
      ...
      try {
        const res = await api.get("api/item/get", { params: this.params });
