			...
			<div class="d-flex w-100 top-0 position-fixed bg-white bg-opacity-75" style="z-index: 99">
				<nav class="navbar hstack justify-content-between w-75 p-md-4 px-md-5 mx-auto">
					...
					<ul class="nav gap-2">
						...
						<li class="nav-item" v-if="token && user?.role === '0'">
							<router-link :to="{ name: 'AdminDashboard' }" class="nav-link"> <i class="bi bi-columns-gap me-2"></i>Dashboard</router-link>
						</li>

						<li class="nav-item dropdown" v-if="token && user && user?.role === '1'">
							<button class="nav-link dropdown-toggle" id="appDropdown" data-bs-toggle="dropdown" aria-expanded="false"><i class="bi bi-app-indicator me-2"></i>App</button>

							<ul class="dropdown-menu dropdown-menu-end mt-2 p-2" aria-labelledby="appDropdown" style="width: 220px">
								<li class="px-2 mb-2 text-muted small">Applications</li>

								<li>
									<router-link class="dropdown-item rounded py-2" :to="{ name: 'LibrarianDashboard' }" @click="closeDropdown"> <i class="bi bi-columns-gap me-3"></i>Dashboard </router-link>
								</li>

								<li>
									<router-link class="dropdown-item rounded py-2" :to="{ name: 'Attendance' }" @click="closeDropdown"> <i class="bi bi-person-check me-3"></i>Attendance </router-link>
								</li>

								<li>
									<router-link class="dropdown-item rounded py-2" :to="{ name: 'Circulation' }" @click="closeDropdown"> <i class="bi bi-arrow-left-right me-3"></i>Circulation </router-link>
								</li>

								<li><hr class="dropdown-divider" /></li>
								<li class="text-center small text-muted">More features coming soon</li>
							</ul>
						</li>
		...
		<div class="position-fixed bottom-0 end-0 card border-0 me-2 rounded-bottom-0 support-card bg-transparent" @mouseenter="openCustomerSupport = true" @mouseleave="!hasSupportInput && (openCustomerSupport = false)">
			<div class="d-flex justify-content-end bg-transparent">
				<div class="card-header bg-prime text-white hstack border-0 clickable p-2 px-4">
					<h6 class="mb-0 text-white"><i class="bi bi-headset me-2"></i>Customer Support</h6>
...
import { thisIsMe, token, user } from "@/stores/auth";
