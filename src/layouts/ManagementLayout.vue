	...
	<div class="d-flex flex-column vh-100 overflow-hidden">
						...
						<div class="container">
							<div class="card">
								<div class="card-body text-center">
									...
									<small class="card-subtitle mb-2 text-body-secondary">{{ user.role === "0" ? "Admin" : user.role === "1" ? `@${user.librarian.username} | Librarian` : "Unknown" }}</small>
								</div>
							</div>
						</div>
						<li class="p-2">
							<h6 class="dropdown-header ms-1 ps-0">Actions</h6>
							<router-link :to="{ name: r.path }" class="dropdown-item p-2 py-3 rounded" type="button" v-for="r in profileCardRoutes" :key="r" data-bs-dismiss="dropdown">
		...
		<div class="d-flex w-100 p-0 h-100 overflow-hidden">
			...
			<div class="d-flex flex-column h-100 bg-white overflow-hidden w-100">
				<div class="d-flex justify-content-between align-items-center p-4 pb-2 mb-0">
					<div class="">
						<h4 class="fw-semibold mb-1">{{ page.title }}</h4>
						<p class="text-muted mb-0">{{ page.description }}</p>
					</div>
					<div class="text-end">
						<h4 class="fw-semibold mb-1">{{ page.time }}</h4>
						<small class="mb-0">{{ page.date }}</small>
					</div>
				</div>
				<main class="container-fluid overflow-hidden h-100 p-0 bg-body">
					<router-view />
				</main>
			</div>
		</div>
...
import { preloadLibrarianCache } from "@/stores/librarianCache";

export default {
	name: "AdminLayout",
	async created() {
		this.loadUser();
		preloadLibrarianCache();
	},
	components: {
		SideBar,
		// AdminHeader,
...
<style>
#app {
	font-family: "Inter", "Open Sans", "Poppins";
}
</style>
