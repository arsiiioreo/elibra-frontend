<template>
	...
	<aside id="sidebar" class="d-flex flex-column h-100 bg-white border-end sidebar overflow-hidden" :class="{ open: sideBarOpen }">
		<!-- Logo/Header -->
		<router-link :to="{ name: 'landing' }" class="d-flex align-items-center p-3 pb-2 text-decoration-none text-black w-100" :class="{ 'justify-content-center': !sideBarOpen }">
		...
		<div class="d-flex align-items-center p-2 sidebar-profile overflow-hidden" :class="[{ open: sideBarOpen }, sideBarOpen ? 'd-flex' : 'd-none']">
			...
			<div class="d-flex flex-column sidebar-text">
				<span class="small text-dark">{{ `Welcome, ${user?.role === "0" ? "Admin" : "Librarian"}!` }}</span>
				<span class="text-prime fw-bold pe-1 text-nowrap">{{ user?.first_name?.split(" ", 1)[0] }}</span>
				<small class="text-muted small" v-if="user?.role === '1'">{{ `@${user?.librarian?.username}` }}</small>
			</div>
		</div>
		...
		<!-- Navigation Links -->
		<nav class="nav-scroll overflow-y-auto overflow-hidden h-100 flex-grow-1 pb-5">
			<div class="nav flex-column" v-for="route in sideBarRoutes" :key="route.name">
				...
				<router-link class="nav-link d-flex align-items-center w-100 p-3 text-secondary" :active-class="[sideBarOpen ? 'active text-success fw-bold bg-body-secondary border-start border-5 border-success ms-2 ps-1' : 'active bg-prime text-white C']" v-for="r in route.child" :key="r.path" :to="{ name: r.path }" exact :title="sideBarOpen ? '' : r.name">
