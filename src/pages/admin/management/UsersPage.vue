<template>
	<div class="hstack gap-3 h-100 align-items-center justify-content-start w-100">
				...
				<div class="card-footer d-flex flex-wrap justify-content-between align-items-center gap-3">
					...
					<div class="d-flex align-items-center gap-2 flex-wrap">
						...
						<select name="page" id="page" class="form-select form-select-sm" style="width: auto" v-model="params.page">
							<option :value="p" v-for="p in pagination.last_page" :key="p">{{ p }}</option>
						</select>
		...
		<div class="card h-100" style="min-width: 400px; max-width: 400px" v-if="showDetails">
			...
			<UserInformationPage :id="selectedId" :data="selectedUser" v-if="selectedId" @update-page="updatePage" @page:changes="refresh" />
		</div>
	</div>

	<div class="modal fade" id="addNewUser" data-bs-backdrop="static">
		<AddNewUserPage @added="refresh" />
	</div>
</template>
...
import { fetchUsers } from "@/stores/adminCache";

import SearchInput from "@/components/inputs/SearchInput.vue";
import UserInformationPage from "./users/UserInformationPage.vue";
import AddNewUserPage from "./users/AddNewUserPage.vue";
import ListPage from "./users/ListPage.vue";

export default {
	name: "LibraryInformationPage",
	components: { ListPage, SearchInput, UserInformationPage, AddNewUserPage },
	created() {
		this.fetchUsers();
	},
	computed: {
	...
	methods: {
		...
		selectedUserId(id) {
			if (this.selectedId === id && this.showDetails === true) return this.updatePage(false);
			this.selectedId = id;
		...
		async fetchUsers() {
			...
			try {
				const res = await fetchUsers(this.params);
		...
		async refresh() {
			storage.users = null;
			this.params.sort = "last_name";
			this.params.order = "asc";
			this.params.status = "";
			this.params.role = "";
			this.params.entries = "25";
			this.params.page = "1";
			this.params.campus = "";
			await this.fetchUsers();
		},
