			...
			<table class="table table-hoverable table-bordered table-striped">
				...
				<tbody>
					...
					<tr v-else v-for="(u, i) in users" :key="i">
						<td>{{ i + 1 }}</td>
						<td>{{ u.patron?.user?.last_name }}</td>
						<td>{{ u.patron?.user?.first_name }}</td>
						<td>{{ u.patron?.id_number }}</td>
						<td>{{ u.description }}</td>
						<td>{{ u.group }}</td>
						<td>{{ u.gender }}</td>
						<td>{{ u.branch?.name }}</td>
...
export default {
	...
	methods: {
		async fetchLogs() {
			...
			this.users = null;
			try {
				const res = await api.get("api/patron/logs", { params: { branch_id: localStorage.getItem("branch_id") } });
